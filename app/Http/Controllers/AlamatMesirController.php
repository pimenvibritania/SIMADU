...
class AlamatMesirController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = AlamatMesir::where('user_id', backpack_auth()->id())->get() ?? null;
            return Datatables::of($data)
                ->addIndexColumn()
                ->editColumn('created_at', function ($request) {
                    return $request->created_at->format('Y-m-d'); // human readable format
                })
                ->editColumn('status', function(AlamatMesir $izin) {
...
//                        <button class='waves-effect btn deepPink-bgcolor delete' data-value='$data->id' ><i class='material-icons'>delete</i>Delete</button>";
    ...
    public function create()
    {
        ...
        $no_surat = Helper::generateId(new AlamatMesir(), 'no_surat', 'K/AM', 4);
        return \view('pages.surat.alamat-mesir.create', [
            'user' => $user,
            'no_surat'   => $no_surat
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'jml_surat'    => 'required',
            'keperluan' => 'required',
            'no_surat' => 'unique:alamat_mesirs'
        ]);

        $name = backpack_user()->name;

        $no_permohonan = Helper::generateId(new AlamatMesir(),
            'no_permohonan',
